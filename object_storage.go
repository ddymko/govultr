...
type ObjectStorageService interface {
	Create(ctx context.Context, clusterID int, label string) (*ID, error)
...
func (o *ObjectStorageServiceHandler) Create(ctx context.Context, clusterID int, label string) (*ID, error) {
	uri := "/v1/objectstorage/create"

	values := url.Values{
		"OBJSTORECLUSTERID": {strconv.Itoa(clusterID)},
	}
	...
	bucketID := new(ID)
	err = o.client.DoWithContext(ctx, req, bucketID)
